...
def analyze_bazaar(raw: dict, min_volume: int = 1000, min_margin: float = 2.0) -> list[dict]:
    ...
    for item_id, data in products.items():
        ...
        buy_price: float = quick.get("buyPrice", 0)   # lowest sell order (what you pay instantly)
        sell_price: float = quick.get("sellPrice", 0)  # highest buy order (what you get instantly)
        ...
        # Place buy order at sell_price, sell via sell order at buy_price
        # (we undercut the market slightly to guarantee fills)
